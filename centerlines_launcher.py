...
Frozen-mode launcher for the multi-floor GUI. DXF files remain external
and are selected from the GUI. The builder runs in-process so the frozen
EXE never starts itself recursively via sys.executable.
"""
...
def main() -> int:
    ...
    # Critical EXE fix: replace the source-mode subprocess call. In a
    # PyInstaller executable sys.executable points to CENTERLINES.exe;
    # launching it from save_and_build would reopen the GUI recursively.
